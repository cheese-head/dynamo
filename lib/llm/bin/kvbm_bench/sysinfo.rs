...
fn row(label: &str, value: &str) {
    let content = format!("  {:<15} {}", label, value);
    let truncated: String = content.chars().take(INNER).collect();
    print!("║{:<w$}║", truncated, w = INNER);
    println!();
}

fn section(title: &str) {
    println!("╠{:═<w$}╣", "", w = INNER);
    print!("║  {:<w$}║", title, w = INNER - 2);
    println!();
}

pub fn print_system_info(bench_dir: &str) {
    println!("╔{:═<w$}╗", "", w = INNER);
    print!("║  {:<w$}║", "System Information", w = INNER - 2);
    println!();
...
fn print_host_info() {
    ...
    row("OS:", &format!("{os} (kernel {kernel})"));

    let (cpu_model, cpu_count) = read_cpu_info();
    row("CPU:", &format!("{cpu_model} ({cpu_count} threads)"));

    let mem_gb = read_mem_total_gb();
    row("Memory:", &format!("{mem_gb:.1} GB"));
}
        ...
        .output()
    {
        if let Ok(stdout) = String::from_utf8(output.stdout) {
            for (i, line) in stdout.lines().enumerate() {
                ...
                if parts.len() >= 3 {
                    if i == 0 {
                        row("GPU 0:", &format!("{} ({}MB)", parts[0].trim(), parts[1].trim()));
                        row("Driver:", parts[2].trim());
                    }
                }
            }
        }
    ...
    if ib_path.exists() {
        if let Ok(entries) = fs::read_dir(ib_path) {
            for entry in entries.flatten() {
                ...
                row(&format!("{name}:"), &format!("{board_id} {rate} {pcie}"));
            }
        }
    }
    ...
    if let Ok(entries) = fs::read_dir(net_path) {
        for entry in entries.flatten() {
            ...
            if operstate == "up" || operstate == "unknown" {
                row(&format!("{name}:"), &format!("{speed}Mb/s MTU {mtu} ({operstate})"));
            }
        }
    }
}
...
fn print_pcie_info() {
    ...
    if let Ok(entries) = fs::read_dir("/sys/class/nvme") {
        for entry in entries.flatten() {
            ...
            if !model.is_empty() {
                row(&format!("{name}:"), &format!("{model} @ {addr}"));
            }
        }
    }
}
...
fn print_storage_info(bench_dir: &str) {
    ...
    if let Ok(mounts) = fs::read_to_string("/proc/mounts") {
        ...
        if let Some((dev, _mount, fstype, opts)) = best_match {
            row("Filesystem:", &format!("{dev} ({fstype})"));
            let short_opts: String = opts.chars().take(55).collect();
            row("Mount opts:", &short_opts);
        }
    }
    ...
    if let Ok(stat) = nix::sys::statvfs::statvfs(bench_dir) {
        ...
        row("Total/Avail:", &format!("{total:.1}T / {avail:.1}T"));
    }
}
